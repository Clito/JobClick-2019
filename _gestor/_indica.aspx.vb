...
Partial Class _gestor_indica
    ...
    Protected Sub ImageButtonPsq_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles ImageButtonPsq.Click
        ...
        msn.Text = ""
        VERIFICA_INDICACAO(LabelCPF.Text)

        GridViewPSQProf.Visible = True
    End Sub

    Sub VERIFICA_INDICACAO(ByVal CPF As String)
        ...
        cmd = New SqlCommand("_USP_PESQUISA_PROFISSIONAL_CFP", cn)
    ...
    Protected Sub GridViewVAGA_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles GridViewVAGA.SelectedIndexChanged
        ...
        ProtocoloIndTextBox.BackColor = Drawing.Color.GreenYellow
    End Sub
    ...
    Protected Sub ButtonCadastra_Click(sender As Object, e As System.EventArgs) Handles ButtonCadastra.Click
        ...
        prm.Direction = ParameterDirection.Input
        cmd.Parameters.Add(prm)
        cmd.Parameters("@idCadastroEmpresa").Value = Session("idCadastroEmpresa_Pai")

        prm = New SqlParameter("@idCadastroEmpresaFilha", SqlDbType.Int)
        prm.Direction = ParameterDirection.Input
        cmd.Parameters.Add(prm)
        cmd.Parameters("@idCadastroEmpresaFilha").Value = Session("idCadastroEmpresa_Filha")
        ...
        Try
            Dim dr As SqlDataReader = cmd.ExecuteReader
            PanelAviso.Visible = True
            AVISOLABEL.Text = "• Indicação registrada, e-mail enviado ao profissional.<br>"
            AVISOLABEL.CssClass = "textoProcessoSeletivoABERTO"
            ImageAVISO.ImageUrl = "~/_gestor/images/buttonOK.png"
            dr.Close()
        Catch ex As Exception
            msn.Text = ErrorToString()
            PanelAviso.Visible = True
            AVISOLABEL.Text = "• Falha ao registrar a indicação.<br>"
            AVISOLABEL.CssClass = "textoProcessoSeletivoFECHADO"
            ImageAVISO.ImageUrl = "~/_gestor/images/buttonNOK.png"
        End Try
