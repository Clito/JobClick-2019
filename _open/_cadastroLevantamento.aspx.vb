...
Partial Class _empresa_cadastroLevantamento
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        Select Case HttpContext.Current.User.Identity.Name
            Case "RHUser"
                SqlListaPerfil.SelectParameters("idRH_Gestor").DefaultValue = 0
            Case "Consultor"
                SqlListaPerfil.SelectParameters("idRH_Gestor").DefaultValue = Session("idRH_Gestor")
            Case "Consultor Wizard"
                SqlListaPerfil.SelectParameters("idRH_Gestor").DefaultValue = Session("idRH_Gestor")
        End Select

        Dim AvisoVisualizado As String = Session("AvisoVisualizado")
        If AvisoVisualizado Is DBNull.Value Then
            Session("AvisoVisualizado") = 0
        End If
        ...
        If Not IsPostBack Then
            ...
            If Session("liberaQtdCad") <= 0 Then
                NavActions.Visible = False
                Session("AvisoVisualizado") = 1
            Else
    ...
    Protected Sub DropDownListConsultor_DataBound(sender As Object, e As System.EventArgs) Handles DropDownListConsultor.DataBound
        ...
        myDropDownListConsultor.Items.Insert(0, New ListItem("Atribuir a outro Consultor", 0))
    ...
    Protected Sub InsertButton_Click(sender As Object, e As System.EventArgs) Handles InsertButton.Click
        ...
        Try
            ...
            msnLabel.Text = "O Levantamento do Perfil da Vaga foi salvo com sucesso!"
            LabelCabec.Text = "Levantamento do Perfil da Vaga (Salvo!)"
            ImageButtonVaga.Visible = True
            FormPerfilOut.Visible = False
        Catch ex As Exception
            msnLabel.CssClass = "msn"
            msnLabel.Text = "Falha ao salvar o Levantamento do Perfil da Vaga, avise o Administrador."
            LabelCabec.Text = "Levantamento do Perfil da Vaga (Falha!)"
            ImageButtonVaga.Visible = False
        End Try
    ...
    Protected Sub ButtonAtribuir_Click(sender As Object, e As System.EventArgs) Handles ButtonAtribuir.Click
        ...
        If CInt(DropDownListConsultor.SelectedItem.Value) > 0 And idPerfilVagaLabel.Text <> "" Then
            ...
            prm.Direction = ParameterDirection.Input
            cmd.Parameters.Add(prm)
            cmd.Parameters("@idRH_Gestor").Value = CInt(DropDownListConsultor.SelectedItem.Value)
            ...
            InsertButton.Visible = False
            LinkButtonAlterar.Visible = True
            Response.Redirect("_cadastroLevantamento.aspx?autoTit=Levantamento " & ProtocoloLabel.Text & " foi atribuído a outro Consultor.")
        Else
            msnLabel.Text = "É necessário informar qual consultor será o responsável pelo levantamento.<br>Não é possível atribuir este levantamento a outro consultor neste momento!"
            GridViewListaPerfil.DataBind()
            FormPerfilOut.Visible = True
            InsertButton.Visible = False
            LinkButtonAlterar.Visible = True
    ...
    Protected Sub ImageButtonVaga_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles ImageButtonVaga.Click
        Response.Redirect(Session("url") & "/" & "/_cadastraVaga.aspx?autoTit=Cadastro%20de%20novos%20anúncios")
    End Sub
End Class
